...
** delete text when it's marked by typing
  ...
  #+end_src
** minibuffer
I just want to use consult-yank-pop in the minibuffer
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (set-frame-font "Monospace-13" nil t) ;; only this line seems to have worked
...
**** The difference is hard to measure in small buffers, but in large files (think multi-thousand-line JSON or log files) it adds up
#+begin_src emacs-lisp
...
#+end_src
**** execute function from current line to line n
#+begin_src emacs-lisp
  (defun line-region-bounds (n)
    "Return (BEG . END) spanning from the current line to line N (inclusive)."
    (let* ((current (line-number-at-pos))
           (start   (min current n))
           (end     (max current n)))
      (cons
       (save-excursion (goto-line start) (line-beginning-position))
       (save-excursion (goto-line end)   (line-end-position)))))

  (defun region-action-to-line (action n)
    "Apply ACTION (a function taking BEG and END) from current line to line N.
  ACTION receives two arguments: the beginning and end of the resolved region."
    (let* ((bounds (line-region-bounds n))
           (beg    (car bounds))
           (end    (cdr bounds)))
  ...
  (defmacro define-region-dispatch-cmd (name action)
    `(defun ,name ()
       (interactive)
       (region-action-to-line ,action (read-number "To line: "))))

  (define-region-dispatch-cmd region-dispatch-comment #'comment-or-uncomment-region)
  (define-region-dispatch-cmd region-dispatch-kill    #'kill-region)
  (define-region-dispatch-cmd region-dispatch-yank    #'kill-ring-save)
  (define-region-dispatch-cmd region-dispatch-indent  #'indent-region)
  (define-region-dispatch-cmd region-dispatch-upcase  #'upcase-region)
  (define-region-dispatch-cmd region-dispatch-downcase #'downcase-region)

  (transient-define-prefix region-dispatch-menu ()
    "Dispatch a region action from current line to line N."
    ["Region Actions"
     (";" "comment/uncomment" region-dispatch-comment)
     ("k" "kill (cut)"        region-dispatch-kill)
     ("y" "yank (copy)"       region-dispatch-yank)
     ("i" "indent"            region-dispatch-indent)
     ("u" "upcase"            region-dispatch-upcase)
     ("d" "downcase"          region-dispatch-downcase)])

  (global-set-key (kbd "C-c r x") #'region-dispatch-menu)
#+end_src
...
both stolen from [[https://github.com/daedreth/UncleDavesEmacs/blob/master/config.org#following-window-splits][UncleDaves's config]]
  ...
  #+end_src
** resize window combinations proportionally automatically
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c o s i") 'org-insert-subheading)
  (global-set-key (kbd "C-c o e u") 'org-up-element)
  (global-set-key (kbd "C-c o e d") 'org-down-element)
  (global-set-key (kbd "C-c '") 'org-edit-src-code)

  (defun my-org-insert-heading-after-current ()
    "Insert a new heading at the same level on the line immediately
  below the current heading, regardless of cursor position or subtree."
    (interactive)
    (org-back-to-heading)
    (let ((level (org-current-level)))
      (end-of-line)
      (insert "\n" (make-string level ?*) " ")))
  (global-set-key (kbd "C-c o RET") 'my-org-insert-heading-after-current)
...
#+begin_src emacs-lisp
  (use-package org-roam
    :ensure t
    :custom
    (org-roam-directory "~/Documents/personal/professionalHouseOnPersonalLand/programming/orgRoamNotes")
...
TODO: avy-goto-char-timer + symbol-at-point = could make it easier to jump to a non-highlighted part of a frame
  #+begin_src emacs-lisp
    (use-package avy
      ...
      (defun avy-action-goto-end-of-line (pt)
        (goto-char pt)
        (end-of-line)
        t)
      ...
      (setf (alist-get ?k avy-dispatch-alist) 'avy-action-kill-stay
            ...
            (alist-get ?e avy-dispatch-alist) 'avy-action-goto-end-of-line)
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    (defun my-magit-checkout-and-pull-from-master ()
      (interactive)
      (if (magit-anything-modified-p)
          (user-error "You have staged or unstaged changes. Not ready to check out master")
        (let ((base-branch (magit-read-branch "Select base branch (default=master)" "master")))
          (unless (string-equal-ignore-case (magit-get-current-branch) base-branch)
            (message "Not on base branch '%s'. Checking out '%s'" base-branch base-branch)
            (magit-branch-checkout base-branch))
          ;; pull from base-branch
          (magit-pull-from-upstream nil))))

    (defun my-magit-create-new-branch-off-master ()
      (interactive)
      (if (magit-anything-modified-p)
          (user-error "You have staged or unstaged changes. Not ready to check out master and create new branch")
        (let ((base-branch (magit-read-branch "Select base branch (default=master)" "master")))
          (unless (string-equal-ignore-case (magit-get-current-branch) base-branch)
            (message "Not on base branch '%s'. Checking out '%s'" base-branch base-branch)
            (magit-branch-checkout base-branch))
...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    :bind (:map projectile-command-map
                ("h g" . my-projectile-switch-and-grep))
    :config
...
** sql
#+begin_src emacs-lisp
  ;; aggressive-indent-mode doesn't seem to be working right in sql-mode
...
#+begin_src emacs-lisp
  (setq treesit-language-source-alist
        '((bash "https://github.com/tree-sitter/tree-sitter-bash")
          ...
          ;; yaml-ts-mode not working. Can't be compiled
...
#+begin_src emacs-lisp
  (setq major-mode-remap-alist
        '((bash-mode . bash-ts-mode)
          ...
          ;; yaml-ts-mode not working - something with gcc, libc, or ld
          ;; Warning (treesit): Error encountered when installing language grammar: (treesit-error Command: /snap/emacs/3130/usr/bin/g++-14 -fPIC -shared --sysroot /home/ckelly/snap/emacs/common/sysroot/ -B /home/ckelly/snap/emacs/common/sysroot/usr/lib/gcc parser.o scanner.o -o libtree-sitter-yaml.so Error output: /snap/emacs/3130/usr/bin/ld: cannot find /lib/x86_64-linux-gnu/libm.so.6: No such file or directory
          ;; /snap/emacs/3130/usr/bin/ld: cannot find /lib/x86_64-linux-gnu/libmvec.so.1: No such file or directory
...
*** TODO get bash-ts-mode working on work laptop's RHEL workspace
#+begin_src emacs-lisp
  (use-package treesit-auto
    :custom
    (treesit-auto-install 'prompt)
    ;; bash is NOT working right now with natively-compiled emacs on my work laptop's RHEL workspace
    ;; Remove bash from this list BEFORE saying, "download all" (treesit-auto-add-to-auto-mode-alist -> below)
    (treesit-auto-langs (cl-remove 'bash treesit-auto-supported-languages))
    :config
...
#+end_src
* AI
** [[https://gptel.org/manual.html][gptel]]
*** [[https://github.com/karthink/gptel][github]]
**** [[https://github.com/karthink/.emacs.d/blob/master/lisp/setup-gptel.el][karthink's gptel config]]
**** [[https://github.com/gregoryg/emacs-gregoryg/blob/cc75a1d6ad289e2a415ef2ffafe318e51c556245/AI.org][another config to look at]]
*** to key api keys from:
**** [[https://platform.claude.com/settings/keys][anthropic (claude)]]
**** [[https://platform.openai.com/api-keys][openAI]]
**** [[https://openrouter.ai/settings/keys][OpenRouter]]
*** config
**** TODO figure out which models are free for me to use and add those to the "OpenRouter" backend
**** Might need to use a free model from [[https://openrouter.ai/models?order=pricing-low-to-high][OpenRouter]]
#+begin_src emacs-lisp
  ;; helper function to get api keys
  (defun read-first-line-of-file (file)
  ...
  (use-package gptel
    ...
    :bind (("C-c g s" . gptel-send)
           ("C-c g m" . gptel-menu)
           ("C-c g <return>" . gptel)
           ("C-c g C-g" . gptel-abort)
           ("C-c g +" . gptel-add)))

  (gptel-make-anthropic "Claude-thinking"
    :key (lambda () (read-first-line-of-file "~/.emacs.d/claude-api-key-free.txt"))
    :stream t
    :models '(claude-sonnet-4-6
              claude-sonnet-4-5-20250929)
    :request-params '( :thinking (:type "enabled")))

  (gptel-make-openai "OpenAI-ChatGPT"
    :key (lambda () (read-first-line-of-file "~/.emacs.d/open-ai-key-free.txt"))
    :stream t
    :models '(gpt-5.2
              gpt-4.1
              gpt-4.1-mini
  ...
  (gptel-make-openai "OpenRouter"
    ...
    :key (lambda () (read-first-line-of-file "~/.emacs.d/open-router-api-key-free.txt"))
    :models '(
              qwen/qwen3-coder:free                     ; best free coding model
              deepseek/deepseek-r1:free                 ; strong reasoning/coding
              meta-llama/llama-3.3-70b-instruct:free    ; solid general use
              openrouter/free                           ; auto-picks best available free model
              ;; paid but cheap if you ever add credits:
              deepseek/deepseek-chat                    ; V3, very cheap
              anthropic/claude-sonnet-4-6               ; requires OR credits
              ))
  ...
  ;; ERRORS
  ;;Claude-thinking error: ((HTTP/2 400) invalid_request_error) Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.
  ;;
  ;; OpenAI-ChatGPT error: ((HTTP/2 401) invalid_request_error) You didn't provide an API key. You need to provide your API key in an Authorization header using Bearer auth (i.e. Authorization: Bearer YOUR_KEY), or as the password field (with blank username) if you're accessing the API from your browser and are prompted for a username and password. You can obtain an API key from https://platform.openai.com/account/api-keys.
#+end_src
* Miscellaneous
...
#+end_src
* NOTES
** To get this to work on my work laptop's RHEL Workspace
*** changed this to just setq in init.el
#+begin_example
   ;; '(package-archives
...
#+end_example
*** had to go into smart-tabs-mode.el and move a macro
**** did what [[https://github.com/jcsalomon/smarttabs/pull/54/files][this PR]] did: move smart-tabs-create-advice-list macro up
*** changed the line-number-current-line face from being bold. This was causing an annoying visual effect where the current line was moving slightly to the right
#+begin_example
  ;;   '(line-number-current-line ((t (:inherit (hl-line default) :foreground "#D85F00" :weight normal))))
#+end_example
*** changed the font to be monospace so lines/columns would align
#+begin_example
    ;; (set-frame-font "Monospace-11" nil t)
#+end_example
*** Snap
**** snap emacs couldn't compile treesitter for yaml. Some compatibility issue with snap emac's ld command not being able to find the host's libm or libmvec files. Idk
***** solution:
****** Use [[https://github.com/casouri/tree-sitter-module/tree/master][this repo's]] build.sh script to compile libtree-sitter-yaml.so, then copy to `~/.emacs.d/tree-sitter/`
****** Not the best solution since we're compiling libtree-sitter-yaml outside of our compiled emacs and expecting it to work, but hey. It worked.
***** research:
****** [[https://github.com/alexmurray/emacs-snap/issues/103][emacs-snap issue 103]]
****** [[https://github.com/alexmurray/emacs-snap/issues/77][emacs-snap issue 77]]
****** [[https://github.com/alexmurray/emacs-snap/issues/66][emacs-snap issue 66]]
****** [[https://unix.stackexchange.com/questions/506866/what-should-i-install-to-correct-ld-cannot-find-lgbm-and-linput-so-that-i-c]["ld cannot find" issue]] and [[https://stackoverflow.com/questions/16710047/usr-bin-ld-cannot-find-lnameofthelibrary][another "ld cannot find" issue]]
***** full error:
#+begin_example
  Warning (treesit): Error encountered when installing language grammar: (treesit-error Command: /snap/emacs/3130/usr/bin/g++-14 -fPIC -shared --sysroot /home/ckelly/snap/emacs/common/sysroot/ -B /home/ckelly/snap/emacs/common/sysroot/usr/lib/gcc parser.o scanner.o -o libtree-sitter-yaml.so Error output: /snap/emacs/3130/usr/bin/ld: cannot find /lib/x86_64-linux-gnu/libm.so.6: No such file or directory
  /snap/emacs/3130/usr/bin/ld: cannot find /lib/x86_64-linux-gnu/libmvec.so.1: No such file or directory
...
#+begin_example
  ...
  sudo dnf install gtk3-devel libXpm-devel giflib libjpeg-turbo-devel libtiff-devel gnutls-devel ncurses-devel libxml2-devel jansson-devel harfbuzz-devel  libgccjit-devel
  sudo dnf install libtree-sitter-devel librsvg2-devel

  wget https://ftp.gnu.org/gnu/emacs/emacs-30.2.tar.xz
  tar -axvf emacs-30.2.tar.xz -C <extract_to_wherever_you_want>

  cd into emacs-30....


  ./configure --with-native-compilation --with-tree-sitter --with-json --with-modules --with-rsvg --with-gif=ifavailable
...
***** https://phoenixnap.com/kb/emacs-linux
**** errors and Warnings
#+begin_example
  ⛔ Warning (treesit): The installed language grammar for bash cannot be located or has problems (version-mismatch): 15
  ⛔ Warning (native-compiler): er-basic-expansions.el:253:23: Warning: the function ‘treesit-node-at’ is not known to be defined.
  ⛔ Warning (native-compiler): er-basic-expansions.el:252:25: Warning: the function ‘treesit-node-on’ is not known to be defined.
  ⛔ Warning (native-compiler): er-basic-expansions.el:250:12: Warning: the function ‘treesit-language-at’ is not known to be defined.
  ⛔ Warning (emacs): Error retrieving icon script-text-outline, falling back on short-text
